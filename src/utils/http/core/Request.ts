...
export default class Request {
  ...
  middleware<T>(config: HttpRequestConfig): Promise<T> {
    config = mergeConfig(this.config, config);
    const chain = [dispatchRequest, undefined];
    const response: T = {
      config: config,
    } as T;
    let promise = Promise.resolve(response) as Promise<T>;
    ...
    while (chain.length) {
      promise = promise.then<T>(chain.shift(), chain.shift());
    }
  ...
  get<T>(url: string, options: Partial<HttpRequestConfig> = {}): Promise<T> {
