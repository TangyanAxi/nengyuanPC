<template>
  <view>
    ...
    >
      ...
      >
        <view @click="itemClick(index)" class="wrap" :style="{ width: showNum == 5 && list.length >= 5 ? '84%' : '88%' }">
          <image :id="index" class="image" :src="isObj(item) ? item.imgUrl : item" />
    ...
    <view class="sw-con" v-if="showDot">
      <view class="t-dot-con">
        <view class="t-dot-active" :style="{ left: 40 * curr + 'rpx', background: dotSelectedColor }" />
        <block v-for="(dot, idx) in list.length" :key="idx">
          <view class="t-dot-wrap">
            <view :class="['t-dot', { active: idx == curr }]" :style="{ background: dotColor }" />
          </view>
        </block>
      </view>
    </view>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      touchEnd(e) {
        let len = this.styleList.length;
        let touchIdx = Number(e.target.id);
