        ...
        <scroll-view scroll-x style="width: 100%; white-space: nowrap" :scroll-left="scrollLeft">
          <view v-for="(item, index) in tree_stack" class="inline-item" :key="index">
            ...
            <view v-if="index == 0 && isre" @click="backTree(item, -2)" :class="[index == tree_stack.length - 1 && isre ? 'none' : 'active', 'inline-item']">
              <i class="iconfont icon-z043 iconclass"></i>
              搜索结果
            </view>
            ...
            <view class="list-item" v-show="isCheck">
              ...
              <view class="checkbox" v-if="props.options.multiple" @click.stop="handleClick(item, -1)">
                <span v-if="isSelect(item)">
                  <i v-if="item.bx && newCheckList.length != 0" class="iconfont icon-banxuanzhongshousuo1-shi icons"></i>
                  <i v-else class="iconfont icon-xuanzhong txt icon-selected"></i>
                </span>
                <i v-else-if="item.qx" class="iconfont icon-xuanzhong txt icon-selected"></i>
                <i v-else-if="item.bx" class="iconfont icon-banxuanzhongshousuo1-shi icons"></i>
                <i style="color: #b8b8b8" v-else class="iconfont icon-weixuanzhong txt"></i>
              </view>
              <!-- 单选 -->
              <view class="checkbox" v-else-if="props.options.nodes ? (item.user ? true : false) : true" @click.stop="handleClick(item, -1)">
                <i v-if="radioSelect(item)" class="txt iconfont icon-selected"></i>
                <i style="color: #b8b8b8" v-else class="txt iconfont icon-weixuanzhong1"></i>
              </view>
            </view>
...
<script setup>
  ...
  import { ref, reactive, defineProps, computed, nextTick, defineEmits } from 'vue';
